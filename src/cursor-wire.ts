...
function readLengthDelimited(reader: WireReader): Uint8Array {
	...
	if (!Number.isSafeInteger(length)) {
		throw new Error("length-delimited size is too large");
	}
...
function skipBytes(reader: WireReader, length: number): void {
	const end = reader.offset + length;
	if (end > reader.bytes.length) {
		throw new Error("fixed-width field exceeds buffer");
	}
	reader.offset = end;
}
...
function decodeModelParameter(bytes: Uint8Array): CursorModelParameter {
	...
	while (reader.offset < bytes.length) {
		...
		if (fieldNo === 1 && wireType === 2) {
			parameter.id = decodeString(readLengthDelimited(reader));
...
function decodeParameterizedVariant(bytes: Uint8Array): CursorParameterizedVariant {
	const reader: WireReader = { bytes, offset: 0 };
	const variant: CursorParameterizedVariant = {
		parameters: [],
		isMaxMode: false,
	};
	while (reader.offset < bytes.length) {
		...
		if (fieldNo === 1 && wireType === 2) {
			...
			variant.displayNameOutsidePicker = decodeString(
				readLengthDelimited(reader),
			);
		} else if (fieldNo === 9 && wireType === 2) {
			variant.variantStringRepresentation = decodeString(
				readLengthDelimited(reader),
			);
		} else {
			skipWireField(reader, wireType);
...
function decodeParameterizedModel(bytes: Uint8Array): CursorParameterizedModel {
	...
	while (reader.offset < bytes.length) {
		...
		const wireType = tag & 0x7;
		if (fieldNo === 1 && wireType === 2) {
			model.name = decodeString(readLengthDelimited(reader));
		} else if (fieldNo === 10 && wireType === 0) {
			model.supportsImages = readVarint(reader) !== 0;
		} else if (fieldNo === 14 && wireType === 0) {
			model.supportsMaxMode = readVarint(reader) !== 0;
		} else if (fieldNo === 15 && wireType === 0) {
			model.contextTokenLimit = readVarint(reader);
			...
			model.variants.push(
				decodeParameterizedVariant(readLengthDelimited(reader)),
			);
		} else {
			skipWireField(reader, wireType);
		}
...
export function decodeAvailableModelsResponse(
	bytes: Uint8Array,
